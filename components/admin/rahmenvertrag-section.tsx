...
export function RahmenvertragSection({ 
  clientId,
  url,
  signedAt 
}: { 
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const res = await fetch(`/api/clients/${clientId}/upload-rahmenvertrag`, {
        method: "POST",
        body: formData,
      });
    ...
    <div className="rounded-xl border bg-card text-card-foreground shadow-sm">
      <div className="flex flex-col space-y-1.5 p-6 border-b">
        ...
        <p className="text-sm text-muted-foreground">
          Generieren Sie den Rahmenvertrag oder laden Sie eine unterschriebene Version hoch.
        </p>
      </div>
      <div className="p-6 space-y-6">
        <div className="flex items-center gap-4">
          <div className="flex-1">
            ...
            <p className="text-sm text-muted-foreground">
              Lädt den vorausgefüllten 12-seitigen Rahmenvertrag als PDF herunter.
            </p>
          </div>
          <Button variant="outline" className="gap-2" onClick={() => window.open(`/api/clients/${clientId}/rahmenvertrag`, "_blank")}>
        ...
        <div className="flex items-center gap-4 border-t pt-6">
          <div className="flex-1">
            ...
            <p className="text-sm text-muted-foreground">
              Laden Sie das vom Kunden unterschriebene Dokument hoch, um es im System zu speichern.
            </p>
          </div>
