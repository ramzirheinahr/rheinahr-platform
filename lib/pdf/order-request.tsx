...
export type OrderRequestPdfData = {
  facilityName: string;
  requestLabel: string; // e.g. "Anfrage vom 12.07.2026" or "Anfrage [ID]"
  ...
  return (
    ...
    >
      <Page size="A4" style={styles.page}>
        ...
        <Text style={styles.title}>Anfrage {d.requestLabel}</Text>
        ...
        {d.rows.map((r, i) => {
          const hours =
            r.billing === "confirmed"
              ? r.confirmedHours ?? 0
              : r.billing === "accepted" || r.billing === "assigned" || !r.billing
                ? r.scheduledHours
                : null;
          const tone =
            r.billing === "confirmed"
          ...
          return (
            <View key={r.id || i} style={styles.tr} wrap={false}>
              ...
              <Text style={styles.cQual}>{qualLabel[r.qualification] || r.qualification}</Text>
        ...
        <View style={styles.total} wrap={false}>
          <Text style={{ flex: 1 }}>Gesamtsumme (ohne Pausen)</Text>
        </View>
        {d.totals.acceptedShifts > 0 ? (
          <View style={styles.totalLine} wrap={false}>
            <Text style={styles.totalLabel}>Geplant / Angenommen: </Text>
        ...
        <View style={styles.totalLine} wrap={false}>
          <Text style={styles.totalLabel}>Gesamt (inkl. geplant): </Text>
        ...
        <Text style={styles.footer} fixed>
          ...
          signierten Leistungsnachweisen; geplante / vorläufige Stunden basieren auf
          den Anfragedaten. (DSGVO-konform protokolliert).
        </Text>
      </Page>
    </Document>
  );
